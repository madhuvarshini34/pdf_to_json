...
# Function to safely search for a pattern in a line
def safe_search(pattern, line, group=1, default="Unknown"):
    match = re.search(pattern, line)
    return match.group(group) if match else default

# Function to extract all relevant data from the given PDF
def extract_all_data_from_pdf(pdf_path):
    ...
    current_group = None  # Tracks whether parsing Originator or Beneficiary
    ...
    while lines:
        ...
        for field_name, pattern in fields.items():
            value = safe_search(pattern, line)
            if value != "Unknown":
...
@app.get("/extract-pdf-data/{pdf_file_name}")
async def extract_pdf_data(pdf_file_name: str):
    pdf_folder_path = r"C:\Python313\archieve\pdf_to_json\ref"  

    try:
        # Check if the file exists in the folder
        pdf_path = os.path.join(pdf_folder_path, pdf_file_name)
        if not os.path.exists(pdf_path):
            return {"error": f"File '{pdf_file_name}' not found in the folder."}

        # Process the selected file
        json_data = extract_all_data_from_pdf(pdf_path)

        # Return the extracted data as JSON without 'data' key
        return json_data
